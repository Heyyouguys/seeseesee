  ...
  const fetchAvailableModels = async () => {
    ...
    try {
      ...
      if (data.models && data.models.length > 0) {
        ...
        showMessage('success', `成功获取 ${data.models.length} 个可用模型`);
      } else {
        showMessage('error', '未获取到可用模型，请使用自定义输入');
    ...
    <div className='space-y-6'>
              ...
              <div className='flex items-center gap-4 mb-3'>
                <label className='flex items-center cursor-pointer'>
                  <input
                    ...
                    className='mr-2'
                  />
                  <span className='text-sm text-gray-700 dark:text-gray-300'>从列表选择</span>
                </label>
                <label className='flex items-center cursor-pointer'>
                  <input
                    type='radio'
                    ...
                    className='mr-2'
                  />
                  <span className='text-sm text-gray-700 dark:text-gray-300'>自定义输入</span>
                </label>
              </div>
                ...
                <div className='space-y-3'>
                  <div className='flex gap-2'>
                    <div className='relative flex-1' ref={modelDropdownRef}>
                      <button
                        ...
                        className='w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 flex items-center justify-between'
                      >
                        <span className={aiSettings.model ? '' : 'text-gray-400'}>
                          {aiSettings.model || '请选择模型...'}
                        </span>
                      ...
                      {showModelDropdown && (
                        <div className='absolute z-50 w-full mt-1 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg shadow-lg max-h-60 overflow-y-auto'>
                          {availableModels.length > 0 ? (
                            <>
                              <div className='px-3 py-2 text-xs text-gray-500 dark:text-gray-400 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 sticky top-0'>
                                从API获取的模型 ({availableModels.length}个)
                              </div>
                              {availableModels.map((model, index) => (
                                <button
                                  ...
                                  className={`w-full px-3 py-2 text-left text-sm hover:bg-blue-50 dark:hover:bg-blue-900/30 transition-colors ${aiSettings.model === model ? 'bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300' : 'text-gray-900 dark:text-gray-100'
                                    }`}
                                >
                                  {model}
                                </button>
                              ))}
                              <div className='px-3 py-2 text-xs text-gray-500 dark:text-gray-400 border-t border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900'>
                                常用模型参考
                              </div>
                            </>
                          ) : (
                            <div className='px-3 py-2 text-xs text-gray-500 dark:text-gray-400 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 sticky top-0'>
                              常用模型参考（点击&quot;获取模型&quot;加载API可用模型）
                            </div>
                          )}
                          {MODEL_EXAMPLES.map((model, index) => (
                            <button
                              key={`example-${index}`}
                              type='button'
                              onClick={() => handleSelectModel(model)}
                    ...
                    <button
                      ...
                      className='px-4 py-2 bg-purple-600 hover:bg-purple-700 disabled:bg-gray-400 disabled:cursor-not-allowed text-white rounded-lg font-medium transition-colors flex items-center gap-2 whitespace-nowrap'
                    >
                      {isLoadingModels ? (
                        <>
                  ...
                  {availableModels.length > 0 && (
                    <p className='text-xs text-green-600 dark:text-green-400'>
                      ✅ 已从API获取 {availableModels.length} 个可用模型
                    </p>
                  )}
                </div>
              )}
        ...
        <div className='bg-white dark:bg-gray-800 rounded-lg p-6 border border-gray-200 dark:border-gray-700 shadow-sm'>
          ...
          <div className='mb-6'>
            <label className='flex items-center cursor-pointer'>
              ...
              />
              <div className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${aiSettings.enableOrchestrator
                  ? 'bg-purple-600'
                  : 'bg-gray-200 dark:bg-gray-600'
                }`}>
                <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${aiSettings.enableOrchestrator ? 'translate-x-6' : 'translate-x-1'
                  }`} />
              </div>
              <span className='ml-3 text-sm font-medium text-gray-900 dark:text-gray-100'>
                启用智能协调器（意图分析）
            ...
            <div className='space-y-4 pl-6 border-l-2 border-purple-200 dark:border-purple-800'>
              ...
              <div>
                <label className='flex items-center cursor-pointer'>
                  ...
                  />
                  <div className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${aiSettings.enableWebSearch
                      ? 'bg-green-600'
                      : 'bg-gray-200 dark:bg-gray-600'
                    }`}>
                    <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${aiSettings.enableWebSearch ? 'translate-x-6' : 'translate-x-1'
                      }`} />
                  </div>
                  <span className='ml-3 text-sm font-medium text-gray-900 dark:text-gray-100'>
                    启用联网搜索（Tavily）
                                ...
                                <div className='space-y-2'>
                                  ...
                                  <div>
                                    ...
                                    <div className='h-2 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden'>
                                      <div
                                        className={`h-full rounded-full transition-all ${(usage.keyUsage / usage.keyLimit) > 0.9
                                            ? 'bg-red-500'
                                  ...
                                  <div>
                                    ...
                                    <div className='h-2 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden'>
                                      <div
                                        className={`h-full rounded-full transition-all ${(usage.planUsage / usage.planLimit) > 0.9
                                            ? 'bg-red-500'
      ...
      <div className='flex flex-wrap gap-3'>
        ...
        <button
          onClick={handleSave}
          disabled={isLoading}
          className={`flex items-center px-4 py-2 ${hasUnsavedChanges
