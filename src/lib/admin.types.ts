export interface AdminConfig {
  ...
  UserConfig: {
    ...
    RequireApproval?: boolean; // 是否需要审核才能登录，默认 false
    AutoCleanupInactiveUsers?: boolean; // 是否自动清理非活跃用户，默认 false
    InactiveUserDays?: number; // 非活跃用户保留天数，默认 7
    RegisterRateLimit?: {
      enabled: boolean; // 是否启用注册速率限制
      maxPerHour: number; // 每小时每IP最大注册数，默认 3
    };
    Users: {
      username: string;
      role: 'user' | 'admin' | 'owner';
      banned?: boolean;
      approved?: boolean; // 是否已审核通过，默认 true（兼容旧用户）
      approvedAt?: number; // 审核通过时间戳
      approvedBy?: string; // 审核人用户名
      registerIp?: string; // 注册时的IP地址
